...
class Texture:
    ...
    def __call__(self, face_image, albedo, uv_z, verts, trans_verts, light):
        #preprocess images
        #face_image = face_image[int(0.021*face_image.shape[0]):,int(0.018*face_image.shape[1]):]
                #int(0.05*face_image.shape[1]):int(0.96*face_image.shape[1])]
        ...
        uv_pverts_vis = cv2.cvtColor(uv_pverts.numpy().transpose(0, 2, 3, 1)[0], cv2.COLOR_RGB2BGR)
        #cv2.imshow("uv_pverts", uv_pverts_vis)
        uv_gt = F.grid_sample(torch.from_numpy(face_image), uv_pverts.permute(0,2,3,1)[:,:,:,:2], mode='bilinear')
        uv_gt_vis = cv2.cvtColor(uv_gt.numpy().transpose(0, 2, 3, 1)[0], cv2.COLOR_RGB2BGR)
        #cv2.imshow("uv_gt", uv_gt_vis)
        ...
        #cv2.imshow("uv_texture_res", vis_texture)
        #cv2.waitKey(0)
